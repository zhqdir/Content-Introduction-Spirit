...
import { getUserStatus } from '../middleware/usageLimit';
import { setUserApiKey } from '../db/userState';
...
router.post('/api-key', async (req: Request, res: Response) => {
  ...
  try {
    await setUserApiKey(deviceId, apiKey);
    ...
    res.status(500).json({ error: '设置API Key失败' });
  }
});
...
router.get('/remaining', async (req: Request, res: Response) => {
  ...
  try {
    const { getUserRemaining } = await import('../db/userState');
    const remaining = await getUserRemaining(deviceId);
    res.json(remaining);
  } catch (error) {
    res.status(500).json({ error: '获取剩余次数失败' });
  }
});
