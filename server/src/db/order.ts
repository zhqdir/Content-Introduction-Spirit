...
export async function reviewOrder(id: string, approved: boolean, reviewerNote?: string): Promise<Order | undefined> {
  if (!isDatabaseAvailable()) {
  ...
  const result = await pool.query(
    `UPDATE orders SET status = $1, reviewed_at = $2, reviewer_note = $3, updated_at = $4
...
export async function getOrderStats() {
  if (!isDatabaseAvailable()) {
    ...
    const totalAmount = allOrders.filter(o => o.status === 'approved').reduce((sum, o) => sum + o.amount, 0);
...
function reviewOrderInMemory(id: string, approved: boolean, reviewerNote?: string): Order | undefined {
