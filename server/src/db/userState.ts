...
export async function updateUserState(deviceId: string, updates: Partial<UserState>): Promise<void> {
  if (!isDatabaseAvailable()) {
...
export async function activatePremium(deviceId: string, afdianUsername?: string, days: number = 30): Promise<void> {
  if (!isDatabaseAvailable()) {
...
function setUserApiKeyInMemory(deviceId: string, apiKey: string): void {
  let state = memoryUserStates.get(deviceId);
  if (!state) {
    state = { deviceId, ttsUsed: 0, llmUsed: 0, lastResetDate: new Date().toISOString().split('T')[0] };
  }
  state.apiKey = apiKey;
  memoryUserStates.set(deviceId, state);
}
